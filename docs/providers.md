...
The provider-specific Responses API transport and tool decisions are tracked separately in the [Responses compatibility matrix](responses-api.md). Native non-streaming OpenAI transport is enabled; Vertex AI and Bedrock translations remain gated until their item mappings pass conformance tests.
