...
- [ ] Implement the OpenAI-compatible Responses API as a first-class, item-oriented protocol (detailed execution plan: `PLAN.md`, Steps 20–27).
  ...
  - Step 27 has begun with a default-off route flag, a dated machine-readable claim manifest, executable provider preflight checks, and explicit exclusion of unwired streaming and translated providers. Official SDK suites, scheduled upstream drift reporting, recovery/security/billing/load qualification, guardrail metrics, and rollout evidence remain pending.
